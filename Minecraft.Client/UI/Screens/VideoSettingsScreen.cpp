...
void VideoSettingsScreen::buttonClicked(Button *button)
{
    ...
    if (button->id < 100 && (dynamic_cast<SmallButton *>(button) != NULL))
    {
        ...
        Screen::init(minecraft, ssc.getWidth(), ssc.getHeight());
        return;
    }
    if (button->id == 200)
    {
...
void VideoSettingsScreen::render(int xm, int ym, float a)
{
    ...
    {
        SlideButton* slider = dynamic_cast<SlideButton*>(buttons[i]);
        
        if (slider && slider->sliding)
        {
            const Options::Option* op = Options::Option::getItem(slider->id);
            options->set(op, slider->value);
            slider->msg = options->getMessage(op);
        }
    }
